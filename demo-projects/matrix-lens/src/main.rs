...
fn parse_args() -> Result<DemoArgs, Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: cargo run -- [--frames N] [--mode matrix|glass|thermal|xray] [--capture auto|dmabuf|video|pattern] [--resolution 540p|720p|1080p|WIDTHxHEIGHT] [--fps-limit FPS|uncapped]"
                );
                std::process::exit(0);
            }
