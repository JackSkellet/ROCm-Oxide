...
//!
//! ```text
//! demo-projects/path-reconstruction/src/bin/vulkan_interactive_path_reconstruction_motion_denoise_v2_gpuarray.rs
//! ```
//!
//! Run:
//!
//! ```sh
//! ROCM_OXIDE_VISUAL_PRESENT=vulkan cargo run --bin vulkan_interactive_path_reconstruction_motion_denoise_v2_gpuarray
//! ```
//!
//! Bounded run:
//!
...
//!
//! This copy uses `GpuArray<T>` for the host-owned frame, accumulation, and
//! parameter buffers. The HIPRTC kernels are intentionally unchanged so this
//! remains a direct A/B comparison with the original demo.
//!
//! Controls:
//!
//! Camera:
...
use rocm_oxide::{Device, DevicePod, GpuArray, LaunchConfig};
use std::f32::consts::PI;
use std::time::Instant;

#[path = "../visual_presenter.rs"]
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let accum = GpuArray::<f32>::zeros(ACCUM_FLOATS)?;
    let frame = GpuArray::<u32>::zeros(PIXELS)?;
    let params = GpuArray::<RenderParams>::zeros(1)?;
    clear_accum(&clear_kernel, &accum)?;

    let mut window = Window::new(
        "ROCm-Oxide Interactive Path Reconstruction GpuArray",
        WIDTH,
        HEIGHT,
        WindowOptions {
            resize: true,
    ...
    println!(
        "interactive_path_reconstruction: WASD camera, 1-4 select objects/light, 5/6/7/8/PageUp/PageDown move selection"
    );
    println!(
        "render: Space reconstruction | R reset | C material | P pause animation | 9/0 exposure | LeftShift fast"
    );
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        let (forward, right, _up) = camera_basis(yaw, pitch);
        let mut reset = false;
        ...
        unsafe {
            for _ in 0..samples_this_frame {
                ...
                params.copy_from_slice(&[render_params])?;
        ...
        if rendered_frames % 30 == 0 {
            ...
            window.set_title(&format!(
                "Interactive Path Reconstruction GpuArray | samples={} spp={} recon={} selected={} material={} aperture={:.3} focus={:.2}",
                sample_index,
                samples_this_frame,
                reconstruction,
                selected_name,
    ...
    println!(
        "interactive_path_reconstruction_gpuarray: rendered {} frame(s), accumulated {} sample(s)",
