use rocm_oxide::{Device, GpuArray, LaunchConfig};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let block_x = 256u32;
    let device_frame = GpuArray::<u32>::zeros(pixel_count)?;
    let device_camera = GpuArray::<f32>::zeros(CAMERA_PARAMS)?;
    let mut camera_params = vec![0.0f32; CAMERA_PARAMS];

    let mut window = Window::new(
        "ROCm-Oxide Raytraced World GpuArray",
        WIDTH,
        HEIGHT,
        WindowOptions {
            resize: true,
    ...
    while window.is_open() && !window.is_key_down(Key::Escape) {
        ...
        device_camera.copy_from_slice(&camera_params)?;
        ...
        if let Err(err) = render_result {
            window.set_title(&format!(
                "ROCm-Oxide Raytraced World GpuArray | render error: {err} | Esc exit"
            ));
            window.update();
        ...
        if elapsed >= Duration::from_millis(500) {
            ...
            window.set_title(&format!(
                "ROCm-Oxide Raytraced World GpuArray | {:.1} FPS | pos {:.1},{:.1} | shadows {} | reflections {} | WASD move, arrows look, 1/2 toggle, R reset, Esc exit | {}",
                fps,
                pos.x,
                pos.z,
                on_off(shadows),
