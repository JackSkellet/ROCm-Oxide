...
fn parse_args() -> Result<DemoArgs, Box<dyn std::error::Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "Usage: cargo run -- [--frames N] [--mode MODE] [--resolution 4k|WIDTHxHEIGHT] [--present cpu|gl|vulkan] [--present-scale 1|2|4] [--fps-limit FPS|uncapped] [--gpu-work ITERATIONS] [--output PATH]"
                );
                std::process::exit(0);
            }
