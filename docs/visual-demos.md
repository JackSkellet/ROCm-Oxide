...
| `demo-projects/path-reconstruction/` | HIPRTC path tracing, reconstruction, denoise, interactive motion variants, and a `GpuArray<T>` comparison binary. | `cd demo-projects/path-reconstruction && ROCM_OXIDE_VISUAL_PRESENT=vulkan cargo run --bin vulkan_path_reconstruction -- --frames 300` |
